...
namespace MonoGame.Framework.WpfInterop
{
    ...
    public abstract class D3D11Host : Image, IDisposable
    {
        ...
        private double _devicePixelFactor; // detected upon creating Backbuffer, is eg. 1.5 if windows desktop zoom is 150% (only when UseDevicePixelSize == true)
        ...
        public static readonly DependencyProperty UseDevicePixelSizeProperty = DependencyProperty.Register(
            "UseDevicePixelSize", typeof(bool), typeof(D3D11Host), new FrameworkPropertyMetadata(
                (sender, args) =>
                {
                    if (sender is D3D11Host host)
                        host._dpiChanged = true;
                }) { DefaultValue = true });

        

        /// <summary>
        /// Default true. Bypasses the Windows desktop zoom setting so the rendering is "crisp" instead of zoomed with AA. This sets the size of the backbuffer to the exact device-pixel-size instead of WPF's device independent pixel (DIP) size.
        /// </summary>
        public bool UseDevicePixelSize
        {
            get => (bool) GetValue(UseDevicePixelSizeProperty);
            set => SetValue(UseDevicePixelSizeProperty, value);
        }
        ...
        /// <summary>
        /// Converts a position in DIPs (WPF coordinates) to a position matching the current backbuffer. (This only differs if UseDevicePixelSize is true)
        /// </summary>
        public Vector GetPositionInViewport(Vector wpfPosition)
        {
            if (UseDevicePixelSize)
            {
                return wpfPosition * _devicePixelFactor;
            }
        ...
        /// <summary>
        /// Calculates the backbuffer size from the WPF control's size. If UseDevicePixelSize is true, this returns device pixel size, else it returns standard WPF (DIP) size of the host control.
        /// </summary>
        internal Size CalculateNeededBackBufferSize()
        {
            if (UseDevicePixelSize)
            {
                var source = PresentationSource.FromVisual(this);
                var transformToDevice = source.CompositionTarget.TransformToDevice;
                var windowSize = (Size)transformToDevice.Transform((Vector)this.RenderSize);
                _devicePixelFactor = windowSize.Width / this.RenderSize.Width;
                return windowSize;
            }

            _devicePixelFactor = 1;
        ...
        private void CreateBackBuffer()
        {
            ...
            var backBufferSize = CalculateNeededBackBufferSize();
            var width = Math.Max((int)backBufferSize.Width, 1);
            var height = Math.Max((int)backBufferSize.Height, 1);
