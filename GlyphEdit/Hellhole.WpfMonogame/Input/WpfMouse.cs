...
namespace MonoGame.Framework.WpfInterop.Input
{
    ...
    public class WpfMouse
    {
        ...
        private MouseState _mouseState, _viewportMouseState;
        ...
        public MouseState GetState() => _viewportMouseState;
        ...
        private void HandleMouse(object sender, MouseEventArgs e)
        {
            ...
            if ((!_focusElement.IsMouseDirectlyOver || _focusElement.IsMouseCaptured) && CaptureMouseWithin)
            {
                ...
                if (_focusElement.IsMouseCaptured)
                {
                    ...
                    if (!hit)
                    {
                        ...
                        UpdateMouseState((ButtonState)e.LeftButton, (ButtonState)e.MiddleButton, (ButtonState)e.RightButton, (ButtonState)e.XButton1, (ButtonState)e.XButton2);
            ...
            SetMouseState(pos.X, pos.Y, m.ScrollWheelValue + (w?.Delta ?? 0), (ButtonState)e.LeftButton, (ButtonState)e.MiddleButton, (ButtonState)e.RightButton, (ButtonState)e.XButton1, (ButtonState)e.XButton2);
        }

        private void SetMouseState(double posX, double posY, int scrollWheelValue, ButtonState leftButton, ButtonState middleButton, ButtonState rightButton, ButtonState xButton1, ButtonState xButton2)
        {
            _mouseState = new MouseState((int)posX, (int)posY, scrollWheelValue, leftButton, middleButton, rightButton, xButton1, xButton2);
            var deviceMousePosistion = _focusElement.GetPositionInViewport(new Vector(posX, posY));
            _viewportMouseState = new MouseState((int)deviceMousePosistion.X, (int)deviceMousePosistion.Y, scrollWheelValue, leftButton, middleButton, rightButton, xButton1, xButton2);
        }

        private void UpdateMouseState(ButtonState leftButton, ButtonState middleButton, ButtonState rightButton, ButtonState xButton1, ButtonState xButton2)
        {
            _mouseState = new MouseState(_mouseState.X, _mouseState.Y, _mouseState.ScrollWheelValue, leftButton, middleButton, rightButton, xButton1, xButton2);
            _viewportMouseState = new MouseState(_viewportMouseState.X, _viewportMouseState.Y, _viewportMouseState.ScrollWheelValue, leftButton, middleButton, rightButton, xButton1, xButton2);
        }
